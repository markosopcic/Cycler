...
$(document).ready(function(){
    ...
    $("#send-more-invites").click(() => {
        $.ajax("/api/invite-more-friends?eventId="+$("#event-id-input")[0].value,{
            method:"POST",
            contentType : "application/json",
            data : JSON.stringify($("#invite-more-friends").val()),
            success : function(response){
                $("#invite-more-friends").val(null).trigger("change");
                alert("Successfuly sent invitations!")
            },
            error: function(err){
    ...
    $("#invite-more-friends").select2({
        ...
        ajax: {
            url: '/api/search-friends-not-invited',
            data: function (params) {
                if (params.term === null || params.term === undefined || params.term === "")
                    params.term = "";
                var query = {
                    term: params.term,
                    eventId: $("#event-id-input")[0].value
                };
